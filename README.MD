...
> and `flake.lock` — see
> [users/sewer/secrets/README.md](users/sewer/secrets/README.md).
...
For SSH key and secrets-repo access, which must be in place *before* the first
rebuild, see [SECRETS.md](SECRETS.md#setup-for-new-machine).

### Git Pre-commit Hook

A `pre-commit` hook prevents committing an `opencode` submodule bump
without a matching `flake.lock` bump (which causes `NAR hash mismatch`
build failures). To activate it in your local clone:
