...
Then read both the `INVESTIGATION_REPORT.md` and `FLOW_REPORT.md` files that are located inside the `opencode-instance-{id}` directory that was automatically created for this claude session. `FLOW_REPORT.md` may be unavailable, so proceed without it if necessary.
