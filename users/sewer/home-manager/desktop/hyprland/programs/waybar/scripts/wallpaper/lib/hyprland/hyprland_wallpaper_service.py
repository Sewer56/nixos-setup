...
class HyprlandWallpaperService:
    ...
    def _execute_wallpaper_command(self, wallpaper_path: Path, monitor_spec: str) -> WallpaperResult:
        ...
        try:
            # Use reload command with monitor specification
            # Syntax: hyprctl hyprpaper reload "monitor_spec,wallpaper_path"
            subprocess.run(['hyprctl', 'hyprpaper', 'reload', f'{monitor_spec},{wallpaper_path}'], 
    ...
    def set_wallpaper_for_monitor(self, wallpaper_path: Path, monitor_name: str, save_state: bool = True) -> WallpaperResult:
